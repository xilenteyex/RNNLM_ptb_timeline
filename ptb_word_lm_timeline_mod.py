...
def run_epoch(session, model, eval_op=None, verbose=False, epoch_no=None):
  ...
  for step in range(model.input.epoch_size):
    ...
    #added by xilenteyex
#    if epoch_no is not None and step % 20 == 0 and step < 1000:
    if epoch_no is not None and step < 2000:
      ...
      logfile_name = 'med_%d_%d' % (epoch_no, step)
      timline_pref = 'kernelLogs/'
      metadata_pref = 'kernelLogs/'
      if step >= 10 and step <= 998:
#      if step <= 990:

        jsonObj = MessageToJson(run_metadata)
        with open('%smetadata_%s.json' % (metadata_pref, logfile_name), 'w') as outfile:
          json.dump(jsonObj, outfile)

        trace = timeline.Timeline(step_stats=run_metadata.step_stats)
        trace_file = open('%stimeline_%s.ctf.json' % (timline_pref, logfile_name), 'w')
        trace_file.write(trace.generate_chrome_trace_format())
        print('wrote logs')

      #sys.exit()
      if step >= 999:
        with open('kernelLogs/timelistLogs.txt', 'w') as f:
          for ts in timeList:
            f.write('%.5f\n' % (ts))
        sys.exit()
...
#    soft_placement = True
    ...
    with open('kernelLogs/org_graph_rnnlm_ptb_%s.json' % (FLAGS.model), 'w') as outfile:
      json.dump(adj_list_graph, outfile)
    with open('kernelLogs/org_graph_notensors_rnnlm_ptb_%s.json' % (FLAGS.model), 'w') as outfile:
  ...
  with tf.Graph().as_default():
    ...
    # added by xilenteyex
    config_proto = tf.ConfigProto(allow_soft_placement=soft_placement, graph_options=tf.GraphOptions(build_cost_model=1))
