...
def run_epoch(session, model, eval_op=None, verbose=False, epoch_no=None):
  ...
  for step in range(model.input.epoch_size):
    ...
    #added by xilenteyex
    if epoch_no is not None and step % 100 == 0 and step < 1000:
      run_metadata = tf.RunMetadata()
      run_options = tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE, output_partition_graphs=True)
      vals = session.run(fetches, feed_dict, options=run_options, run_metadata=run_metadata)
      logfile_name = 'rnnlm_med_%d_%d' % (epoch_no, step)
      timline_pref = 'logs/timeline_logs/'
      metadata_pref = 'logs/metadata_logs/'
